...
source2=https://github.com/lionsom/LXSpecs.git
source3=http://git.qpaas.com/PaasPods/PaasSpecs.git
commitText=""
tag=""
...
localVerifyLib(){
    ...
    if ! pod lib lint --skip-import-validation --allow-warnings --sources="${source1},${source2},${source3}"; then echo -e "${RED}验证失败${NC}🌧🌧🌧"; exit 1; fi
...
remoteVerifyLib(){
    ...
    if ! pod spec lint --skip-import-validation --allow-warnings --sources="${source1},${source2},${source3}"; then echo -e "${RED}验证失败${NC}🌧🌧🌧"; exit 1; fi
    echo -e "${GREEN}验证成功${NC}🚀🚀🚀"
}
...
publishLib(){
    echo -e "${GREEN}\n第六步：准备发布${tag}版本${NC}⏰⏰⏰"
    if ! pod repo push LXSpecs ${SpecName} --allow-warnings --sources="${source1},${source2},${source3}" --allow-warnings --skip-import-validation; then echo -e "${RED}发布${tag}版本失败${NC}🌧🌧🌧"; exit 1; fi
