...
publishLib(){
    echo -e "${GREEN}\n第六步：准备发布${tag}版本${NC}⏰⏰⏰"
    if ! pod repo push LXSpecs ${SpecName} --allow-warnings --sources="${source1},${source2}" --allow-warnings --use-libraries --skip-import-validation; then echo -e "${RED}发布${tag}版本失败${NC}🌧🌧🌧"; exit 1; fi
